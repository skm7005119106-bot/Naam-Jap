...
function renderCertificates(){const el=document.getElementById("certificateList");el.innerHTML=state.certificates.length?state.certificates.slice().reverse().map(c=>{const naamLabel=c.naamId==="all"?(state.lang==="hi"?(CERT_MILESTONES.find(m=>m.id===c.milestoneId)?.titleHi||"Naam Jap"):(CERT_MILESTONES.find(m=>m.id===c.milestoneId)?.titleEn||"Naam Jap")):displayNaam(naamObj(c.naamId));return `<div class="cert-item"><div class="cert"><div class="cert-lotus">🪷</div><h3>Naam Jap<br>Completion Certificate</h3><p class="cert-intro">This acknowledges completion of a self-recorded Naam Jap practice.</p><p class="cert-line">${state.lang==="hi"?"Yeh certificate hai":"This certifies that"}</p><p class="cert-user-name"><b>${escapeHtml(state.profile.name||"Naam Sadhak")}</b></p><p class="cert-line">${state.lang==="hi"?"ne poora kiya":"has completed"}</p><div class="cert-count-big">${c.target.toLocaleString()} <span>Jap</span></div><p class="cert-line">${state.lang==="hi"?"ka":"of"}</p><div class="cert-naam-pill">🙏 ${escapeHtml(naamLabel)}</div><p class="tiny">Completed: ${c.date}</p><p class="cert-num">Certificate ID: ${c.id}</p><button class="secondary small printCert" data-id="${c.id}">${state.lang==="hi"?"Print / Save PDF":"Print / Save PDF"}</button></div></div>`}).join(""):`<div class="card muted">${state.lang==="hi"?"Target poora hone par certificate yahan milega.":"Completed target certificates will appear here."}</div>`;el.querySelectorAll(".printCert").forEach(b=>b.onclick=()=>printCertificate(b.dataset.id))}

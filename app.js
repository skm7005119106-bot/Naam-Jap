...
function renderCertificates(){
 ...
 if(!state.certificates.length){
   el.innerHTML=`<div class="card muted">${state.lang==="hi"?"Bade milestone ya Sankalp poora hone par aapka premium certificate yahan milega.":"Your premium certificate will appear here when a major milestone or Sankalp is completed."}</div>`;
   return;
 }
 el.innerHTML=state.certificates.slice().reverse().map(c=>{
   const m=c.milestoneId?CERT_MILESTONES.find(x=>x.id===c.milestoneId):null;
   const naam=c.naamId==="all"?(m?(state.lang==="hi"?m.titleHi:m.titleEn):"Naam Jap"):displayNaam(naamObj(c.naamId));
   const milestone=m?(state.lang==="hi"?m.titleHi:m.titleEn):"Sankalp Certificate";
   const mala=Math.floor(Number(c.target||0)/108);
   const remainder=Number(c.target||0)%108;
   return `<div class="cert-item"><div class="premium-cert">
     <div class="cert-aura"></div>
     <div class="cert-crown">✦ ✦ ✦</div>
     <div class="cert-brand">NAAM JAP • NAAM SMARAN</div>
     <div class="cert-seal">ॐ</div>
     <h3>Naam Jap<br><span>Completion Certificate</span></h3>
     <div class="cert-subtitle">${escapeHtml(milestone)}</div>
     <p class="cert-intro">${state.lang==="hi"?"श्रद्धा, नियमितता और नाम स्मरण के इस साधना चरण की पूर्णता के लिए यह डिजिटल प्रमाण-पत्र सम्मानपूर्वक प्रदान किया जाता है।":"This digital certificate recognizes the completion of this Naam Smaran milestone with devotion and regular practice."}</p>
     <div class="cert-presented">प्रमाण-पत्र प्रदान किया जाता है</div>
     <div class="cert-name">${escapeHtml(state.profile.name||"Naam Sadhak")}</div>
     <div class="cert-name-caption">Sadhak / साधक</div>
     <div class="cert-naam-label"><span>JAP NAAM</span><strong>${escapeHtml(naam)}</strong></div>
     <div class="cert-achievement-grid">
       <div><small>COMPLETED JAP</small><b>${Number(c.target||0).toLocaleString()}</b></div>
       <div><small>MALA</small><b>${mala.toLocaleString()}</b></div>
       <div><small>MILESTONE</small><b>${escapeHtml(milestone)}</b></div>
       <div><small>DATE</small><b>${escapeHtml(c.date)}</b></div>
     </div>
     <div class="cert-meta"><div><small>Certificate Number</small><b>${escapeHtml(c.id)}</b></div><div><small>Naam Smaran</small><b>${escapeHtml(naam)}</b></div></div>
     <div class="cert-detail-line">${state.lang==="hi"?`कुल उपलब्धि: ${Number(c.target||0).toLocaleString()} Jap · ${mala.toLocaleString()} पूर्ण Mala${remainder?` · ${remainder}/108 अगली Mala`:""}`:`Achievement: ${Number(c.target||0).toLocaleString()} Jap · ${mala.toLocaleString()} complete Mala${remainder?` · ${remainder}/108 toward next Mala`:""}`}</div>
     <div class="cert-number">${state.lang==="hi"?"यह प्रमाण-पत्र Naam Jap app में दर्ज आपकी self-recorded practice का digital record है।":"This certificate is a digital record of the self-recorded practice stored in the Naam Jap app."}</div>
     <div class="cert-footer">श्रद्धा • साधना • नाम स्मरण</div>
     <button class="secondary printCert" data-id="${c.id}">${state.lang==="hi"?"Print / Save Premium Certificate":"Print / Save Premium Certificate"}</button>
   </div></div>`;
 }).join("");
 el.querySelectorAll(".printCert").forEach(b=>b.onclick=()=>printCertificate(b.dataset.id));
}
...
function printCertificate(id){
 ...
 const mala=Math.floor(Number(c.target||0)/108), remainder=Number(c.target||0)%108;
 const w=window.open("","_blank");if(!w){toast(state.lang==="hi"?"Popup allow karein":"Allow popups");return}
 w.document.write(`<html><head><title>Naam Jap Premium Certificate</title><meta name="viewport" content="width=device-width,initial-scale=1"><style>
 @page{size:A4;margin:7mm}*{box-sizing:border-box}body{margin:0;font-family:Georgia,"Times New Roman",serif;background:#eef4fb;color:#172033;padding:8px}.cert{max-width:790px;min-height:1030px;margin:0 auto;background:radial-gradient(circle at 8% 8%,rgba(236,72,153,.18),transparent 22%),radial-gradient(circle at 92% 8%,rgba(37,99,235,.18),transparent 23%),radial-gradient(circle at 90% 92%,rgba(22,163,74,.14),transparent 24%),linear-gradient(145deg,#fffdf7,#f7fbff 48%,#f5fff8);border:5px solid #c79a31;border-radius:30px;padding:48px 42px;text-align:center;position:relative;overflow:hidden;box-shadow:0 18px 45px rgba(15,23,42,.15)}.cert:before{content:"";position:absolute;inset:13px;border:2px solid #e0c16d;border-radius:22px;pointer-events:none}.brand{position:relative;z-index:1;color:#2563eb;font:900 16px system-ui;letter-spacing:4px}.crown{position:relative;z-index:1;color:#b7791f;font-size:34px;letter-spacing:6px;margin:5px 0}.seal{position:relative;z-index:1;margin:5px auto 10px;width:68px;height:68px;border-radius:50%;display:grid;place-items:center;border:3px solid #d6a93d;background:linear-gradient(145deg,#fff7d6,#e9fff2);color:#8b5e20;font:800 38px Georgia}h1{position:relative;z-index:1;margin:3px 0;color:#17324d;font-size:42px;line-height:1.08}h2{position:relative;z-index:1;margin:8px 0;color:#15803d;font-size:25px}.intro{position:relative;z-index:1;color:#52637a;font:17px/1.6 system-ui;max-width:640px;margin:16px auto}.presented{position:relative;z-index:1;font:700 15px system-ui;color:#8b5e20}.person{position:relative;z-index:1;font-size:38px;font-weight:900;color:#172033;margin:8px 0 0}.caption{position:relative;z-index:1;font:700 14px system-ui;color:#64748b;margin-bottom:14px}.naam{position:relative;z-index:1;display:inline-flex;flex-direction:column;gap:4px;padding:12px 26px;border-radius:20px;background:linear-gradient(135deg,#fff1f2,#eff6ff,#ecfdf5);border:2px solid #b7c9e8;color:#1d4ed8}.naam span{font:800 13px system-ui;letter-spacing:2px}.naam strong{font-size:28px;color:#b45309}.count{position:relative;z-index:1;font-size:42px;font-weight:950;color:#b45309;margin:18px 0 3px}.milestone{position:relative;z-index:1;font:900 23px system-ui;color:#15803d}.grid{position:relative;z-index:1;display:grid;grid-template-columns:1fr 1fr;gap:11px;margin:24px 0}.grid div{padding:13px 10px;border:1px solid #d9e2ef;border-radius:16px;background:rgba(255,255,255,.78);font-family:system-ui}.grid small{display:block;font-size:12px;color:#64748b;margin-bottom:5px;font-weight:800}.grid b{font-size:18px;word-break:break-word}.meta{position:relative;z-index:1;display:grid;grid-template-columns:1fr 1fr;gap:11px;margin:12px 0}.meta div{padding:12px 9px;border:1px solid #ead8a8;border-radius:15px;background:#fffaf0;font-family:system-ui}.meta small{display:block;font-size:12px;color:#64748b;margin-bottom:4px}.meta b{font-size:15px;word-break:break-all}.detail{position:relative;z-index:1;margin:14px 0;padding:12px;border-radius:14px;background:#eefcf4;color:#166534;font:700 15px/1.5 system-ui}.id{position:relative;z-index:1;padding-top:14px;border-top:1px solid #e5dfca;color:#475569;font:13px/1.5 system-ui}.footer{position:relative;z-index:1;margin-top:20px;color:#2563eb;font:800 14px system-ui;letter-spacing:1.5px}@media print{body{background:#fff;padding:0}.cert{box-shadow:none;max-width:none;min-height:0;margin:0}}</style></head><body><div class="cert"><div class="brand">NAAM JAP • NAAM SMARAN</div><div class="crown">✦ ✦ ✦</div><div class="seal">ॐ</div><h1>Naam Jap</h1><h2>Completion Certificate</h2><p class="intro">This digital certificate recognizes the completion of a Naam Smaran milestone with devotion and regular practice.</p><div class="presented">प्रमाण-पत्र प्रदान किया जाता है</div><div class="person">${user}</div><div class="caption">Sadhak / साधक</div><div class="naam"><span>JAP NAAM</span><strong>${naam}</strong></div><div class="count">${Number(c.target||0).toLocaleString()} Jap</div><div class="milestone">${escapeHtml(milestone)}</div><div class="grid"><div><small>COMPLETED JAP</small><b>${Number(c.target||0).toLocaleString()}</b></div><div><small>MALA COMPLETED</small><b>${mala.toLocaleString()}</b></div><div><small>COMPLETION DATE</small><b>${escapeHtml(c.date)}</b></div><div><small>MILESTONE</small><b>${escapeHtml(milestone)}</b></div></div><div class="meta"><div><small>Certificate Number</small><b>${escapeHtml(c.id)}</b></div><div><small>Jap Naam</small><b>${naam}</b></div></div><div class="detail">Achievement: ${Number(c.target||0).toLocaleString()} Jap · ${mala.toLocaleString()} complete Mala${remainder?` · ${remainder}/108 toward next Mala`:""}</div><div class="id">Digitally generated by Naam Jap. This certificate is a self-recorded in-app practice record.</div><div class="footer">श्रद्धा • साधना • नाम स्मरण</div></div><script>window.print()<\/script></body></html>`);w.document.close()
}

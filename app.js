...
function renderCertificates(){
 ...
 if(!state.certificates.length){el.innerHTML=`<div class="card muted">${state.lang==="hi"?"Target poora hone par premium certificate yahan milega.":"Completed target certificates will appear here."}</div>`;return}
 el.innerHTML=state.certificates.slice().reverse().map(c=>{
   const m=c.milestoneId?CERT_MILESTONES.find(x=>x.id===c.milestoneId):null;
   const actualNaam=c.naam || (c.naamId && c.naamId!=="all" ? displayNaam(naamObj(c.naamId)) : displayNaam(naamObj(state.selected)));
   const milestone=m?(state.lang==="hi"?m.titleHi:m.titleEn):"Sankalp Certificate";
   const malas=Math.floor(Number(c.target||0)/108);
   return `<div class="cert-item"><div class="premium-cert">
     <div class="cert-crown">✦ ✦ ✦</div><div class="cert-brand">NAAM JAP • NAAM SMARAN</div>
     <h3>Naam Jap<br>Completion Certificate</h3>
     <div class="cert-subtitle">${escapeHtml(milestone)}</div>
     <p class="cert-intro">${state.lang==="hi"?"श्रद्धा, नियमितता और नाम स्मरण के इस साधना चरण की पूर्णता के लिए यह डिजिटल प्रमाण-पत्र प्रदान किया जाता है।":"This digital certificate recognizes the completion of this Naam Smaran milestone with devotion and regular practice."}</p>
     <div class="cert-name">${escapeHtml(state.profile.name||"Naam Sadhak")}</div>
     <div class="cert-naam-label">Jap Naam · ${escapeHtml(actualNaam)}</div>
     <div class="cert-count">${Number(c.target||0).toLocaleString()} Jap</div>
     <div class="cert-milestone">${escapeHtml(milestone)}</div>
     <div class="cert-meta">
       <div><small>Jap Naam</small><b>${escapeHtml(actualNaam)}</b></div>
       <div><small>Completed Jap</small><b>${Number(c.target||0).toLocaleString()}</b></div>
       <div><small>Completed Mala</small><b>${malas.toLocaleString()}</b></div>
       <div><small>Certificate Type</small><b>${escapeHtml(m?"Milestone Certificate":"Sankalp Certificate")}</b></div>
       <div><small>Completed</small><b>${escapeHtml(c.date||todayKey())}</b></div>
       <div><small>Certificate No.</small><b>${escapeHtml(c.id)}</b></div>
     </div>
     <div class="cert-number">${state.lang==="hi"?"यह Naam Jap app द्वारा generated digital record है। इसमें Naam, Jap count, Mala count, milestone, date और unique certificate number दर्ज है।":"Digitally generated by Naam Jap. This record contains the exact Naam, Jap count, Mala count, milestone, completion date and unique certificate number."}</div>
     <div class="cert-footer">श्रद्धा • साधना • नाम स्मरण</div>
     <button class="secondary printCert" data-id="${escapeHtml(c.id)}">Print / Save PDF</button>
   </div></div>`
 }).join("");
...
function printCertificate(id){
 ...
 const m=c.milestoneId?CERT_MILESTONES.find(x=>x.id===c.milestoneId):null;
 const n=c.naam || (c.naamId && c.naamId!=="all" ? displayNaam(naamObj(c.naamId)) : displayNaam(naamObj(state.selected)));
 const milestone=m?(state.lang==="hi"?m.titleHi:m.titleEn):"Sankalp Certificate";
 ...
 *{box-sizing:border-box}
 ...
 <div class="meta"><div><small>Jap Naam</small><b>${naam}</b></div><div><small>Completed Jap</small><b>${c.target.toLocaleString()}</b></div><div><small>Completed Mala</small><b>${malas.toLocaleString()}</b></div><div><small>Completed</small><b>${c.date}</b></div><div><small>Certificate Type</small><b>${escapeHtml(milestone)}</b></div><div><small>Certificate No.</small><b>${escapeHtml(c.id)}</b></div></div>
